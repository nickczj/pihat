...
This file is the entry point for working on this project with Claude Code. Read it first and treat it as the source of truth for design intent, constraints, and prior decisions. When something here conflicts with a quick-search result, **trust this file** and ask before deviating.
...
| EDA tool | KiCad 8 or 9 | User comfortable with it; open format; matches Inkplate/EPDiy reference repos |
...
- [ ] Clone `devbisme/RPi_Hat_Template` and confirm KiCad opens it cleanly
- [ ] Download all PDFs into `reference/`
- [ ] Read `pimoroni/inky/inky/eeprom.py` and document the byte layout in `docs/eeprom-format.md`
- [ ] Confirm panel availability and shipping window from Good Display to Singapore
- [ ] Decide on KiCad version (8 or 9). All collaborators must match.

### Phase 1 — 7.3" HAT
- [ ] Capture schematic from DESPI-C73 PDF as KiCad source
- [ ] Add HAT EEPROM (24LC256 on GPIO 0/1) per HAT spec
- [ ] Add 4 buttons + 2 STEMMA QT
- [ ] Lay out PCB at ~174×123mm to fit Ikea 180×130 frame
- [ ] DRC clean against JLCPCB 2-layer constraints (clearance 5/5 mil minimum)
- [ ] Export gerbers + BOM + placement, order 5 from JLCPCB
...
## Coding/design conventions for Claude Code
...
If this file feels out of date with reality on the ground, fix it before proceeding. The next person reading it (probably future-you with Claude Code) will thank you.
